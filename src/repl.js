    ...
    switch (command) {
      ...
      case 'csv-to-json': {
        ...
        if (!inputFile || !outputFile) {
          throw new InvalidInputError('Usage: csv-to-json --input <file> --output <file>');
        }
        await csvToJson(resolvePath(state.cwd, inputFile), resolvePath(state.cwd, outputFile));
      ...
      case 'json-to-csv': {
        const { input: inputFile, output: outputFile } = args;
        if (!inputFile || !outputFile) {
          throw new InvalidInputError('Usage: json-to-csv --input <file> --output <file>');
        }
        await jsonToCsv(resolvePath(state.cwd, inputFile), resolvePath(state.cwd, outputFile));
        return { cwd: state.cwd, output: `Converted to ${outputFile}` };
      }
      ...
      case 'hash': {
        ...
        if (!inputFile) {
          throw new InvalidInputError('Usage: hash --input <file> [--algorithm sha256|md5|sha512] [--save]');
        }
        ...
        return { cwd: state.cwd, output: `${result.algorithm}: ${result.digest}` };
      }

      case 'hash-compare': {
        const { input: inputFile, hash: hashFile, algorithm } = args;
        if (!inputFile || !hashFile) {
          throw new InvalidInputError('Usage: hash-compare --input <file> --hash <hashfile> [--algorithm sha256|md5|sha512]');
        }
        const algoStr = typeof algorithm === 'string' ? algorithm : 'sha256';
      ...
      case 'encrypt': {
        ...
        if (!inputFile || !outputFile || !password) {
          throw new InvalidInputError('Usage: encrypt --input <file> --output <file> --password <password>');
        }
        await encrypt(
          resolvePath(state.cwd, inputFile),
      ...
      case 'decrypt': {
        const { input: inputFile, output: outputFile, password } = args;
        if (!inputFile || !outputFile || !password) {
          throw new InvalidInputError('Usage: decrypt --input <file> --output <file> --password <password>');
        }
        await decrypt(
          resolvePath(state.cwd, inputFile),
          resolvePath(state.cwd, outputFile),
      ...
      case 'log-stats': {
        const { input: inputFile, output: outputFile } = args;
        if (!inputFile || !outputFile) {
          throw new InvalidInputError('Usage: log-stats --input <file> --output <file>');
        }
        await logStats(
          resolvePath(state.cwd, inputFile),
          resolvePath(state.cwd, outputFile)
        );
        return { cwd: state.cwd, output: `Stats written to ${outputFile}` };
      }
